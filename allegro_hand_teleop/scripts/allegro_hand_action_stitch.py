...
class AllegroHandActionNode():
    def __init__(self):
        ...
        self.g.trajectory.points.append(JointTrajectoryPoint(positions=self.joint_states.position, velocities=self.joint_velocity, time_from_start=rospy.Duration(0)))
        rospy.sleep(1)
    ...
    def joint_cmd_callback(self, msg):
        if (self.first_read):
            ...
            self.actual_time = rospy.Time.now()
            self.g.trajectory.header.stamp = self.actual_time
        ...
        #self.g.trajectory.points = [JointTrajectoryPoint(positions=self.joint_position, velocities=self.joint_velocity, time_from_start=rospy.Duration(actual_time))]
        self.time_from_start += self.step_time
        self.g.trajectory.points.append(JointTrajectoryPoint(positions=self.joint_position, velocities=self.joint_velocity, time_from_start=rospy.Duration(self.time_from_start)))
